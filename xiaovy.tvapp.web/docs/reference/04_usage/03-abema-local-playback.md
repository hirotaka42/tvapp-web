...
対象: **v1.6.1（main マージ + Cloudflare 本番デプロイ済）**。ABEMA のブラウズ・アプリ内再生・番組情報・
シリーズ/シーズン辿りを、ローカルで動かす手順。

> 本番 Azure リゾルバは現状鍵を返さないため、**ローカルはローカルリゾルバ経由**で再生する。
> 本番修正は `HANDOVER.md` の「本番Azure」節を参照（要 az login）。

## 画面でできること（v1.6.1）
- **ホーム上部**: ランキング上位（総合/アニメの1〜3位）を約10件で自動スライドするヒーロー + 右に「いま放送中」（リアルタイムのライブ、クリックで再生）。
- **ビデオランキング**: 実ランキング8棚。カードをクリック→話数解決→アプリ内再生。
- **再生ページ**: 番組情報（シリーズ/シーズン/話数/あらすじ/ジャンル）+ 下部に同シリーズ全話バー（サムネ付き・現在話ハイライト）。
- **シリーズページ** `/service/abema/series/[seriesId]`: シーズンタブ + 話数リスト（各話アプリ内再生）。
- **有料**: 有料（プレミアム）作品は「有料」明示し再生をブロック。再生エラーは原因別に文言を出し分け。
- **ナビ**: 再生中に上部タブを押すと「再生中です。ホームへ移動しますか?」の確認 → OK でホーム遷移（TVER/ABEMA 共通）。
