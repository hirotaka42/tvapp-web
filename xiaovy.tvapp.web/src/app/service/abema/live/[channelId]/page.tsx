...
function AbemaLivePage(props: { params: Promise<{ channelId: string }> }) {
  const { channelId } = use(props.params);
  const { data, error, retry } = useAbemaStream({ type: 'live', channelId });
  const officialUrl = externalAbemaWatchUrl({ kind: 'live', id: channelId });
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="mx-auto w-[95vw] max-w-5xl py-6 text-left">
        ...
        <h1 className="mt-3 text-2xl font-bold">ABEMA Live</h1>
        <p className="mt-2 text-gray-300">{channelId}</p>
      </section>
    </main>
  );
}

export default AbemaLivePage;
